...
- (void)viewDidLoad {
    ...
    [MAGWebImageConfiguration setupSDWebImage];
    ...
    SDWebImageContext *acontext = @{
        MAGWebImageContextUseGlobalImageURLModifierKey : @(NO),
    };
    [self.imageView1 sd_setImageWithURL:staticWebPURL placeholderImage:nil options:0 context:acontext progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
    ...
    MAGWebImageURLModifierBlock modifier = ^NSURL *(__kindof UIView *magView, NSURL * _Nullable originImageURL, SDWebImageContext * _Nonnull context){
        NSString *imageUrl = originImageURL.absoluteString;
        imageUrl = [NSString stringWithFormat:@"%@?%@&width=%.0f&height=%.0f", imageUrl, @"params=12345", magView.magPreferedWidth, magView.magPreferedHeight];
        return [NSURL URLWithString:imageUrl];
    };
    SDWebImageContext *context = @{
        MAGWebImageContextImageURLModifierKey : [modifier copy],
        MAGWebImageContextUseGlobalBackgroundColorKey : @(NO),
    };
    [self.imageView2 sd_setImageWithURL:animatedWebPURL placeholderImage:nil options:0 context:context progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
...
- (void)viewWillLayoutSubviews {
    ...
    self.imageView1.frame = CGRectMake(0, 0, self.view.bounds.size.width, self.view.bounds.size.height / 3);
    self.imageView2.frame = CGRectMake(0, self.view.bounds.size.height / 2, self.view.bounds.size.width, self.view.bounds.size.height / 3);
}
